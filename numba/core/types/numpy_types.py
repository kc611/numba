...
@total_ordering
class NumPyInteger(BaseInteger):
...
class NumPyIntegerLiteral(BaseIntegerLiteral):
    def __init__(self, value):
        self._literal_init(value)
        name = 'Literal[int]({})'.format(value)
        basetype = self.literal_type
        NumPyInteger.__init__(
            self,
            name=name,
...
class NumPyBoolean(BaseBoolean):
    def cast_python_value(self, value):
        return bool(value)


class NumPyBooleanLiteral(BaseBooleanLiteral, NumPyBoolean):

    def __init__(self, value):
        self._literal_init(value)
        name = 'Literal[bool]({})'.format(value)
        NumPyBoolean.__init__(
            self,
            name=name
            )
...
@total_ordering
class NumPyComplex(BaseComplex):
